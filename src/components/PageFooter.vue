<template>
  <footer
    class="border border-t-2 pt-5 pb-3 flex-shrink-0 border-sdb-100 bg-sdb-400"
  >
    <div class="max-w-sm mx-auto px-4 sm:px-6">
      <ul
        class="flex flex-row items-center justify-between my-2 text-xs text-gray-500 space-x-4"
      >
        <li>
...
export default {
  ...
  data() {
    return {
      ...
      defaultRelease: 'https://github.com/sanger/quanthub/releases',
    }
  },
  created() {
    this.provider()
      ...
      try {
        await fetch('REPO')
          ...
          .then((response1) => {
            ...
            response1.includes('github.com/sanger/quanthub-ui/releases')
              ? (this.repo = response1)
